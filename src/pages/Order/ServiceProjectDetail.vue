  ...
  <section>
    ...
    <ul class="order_list">
      <li class="order_item">
        ...
        <span class="order_num">{{orderId}}</span>
      </li>
      <li class="order_item">
        <span class="order_status">订单状态</span>
        <span class="order_ing">{{orderStateName}}</span>
      </li>
    </ul>
      ...
      <ul class="server_list">
        <li class="server_item" v-for="(server,index) in serverArr" :key="index">
          <div class="outer_wrap">
            <div>
              <img :src="`${baseurl}image/${server.prodImagePath}`" class="item_img">
 ...
 import base from '@/api/base'; 
  export default {
    ...
    created() {
      ...
      this.orderStateName=this.$route.query.state;
      order.getOrderServiceDetail({data:this.$route.query.orderId,"requestId":new Date().getTime()}).then((res) => {
               
                          if(res.code==200){  
                               this.serverArr.push(...res.data)
